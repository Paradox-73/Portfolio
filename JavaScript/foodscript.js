...
class Tornpaper {
    ...
    init(e) {
        ...
        (e.grungeScale || 0 == e.grungeScale) && (this.grungeScale = e.grungeScale);
        (e.grungeFrequency || 0 == e.grungeFrequency) && (this.grungeFrequency = e.grungeFrequency);
        (e.tornFrequency || 0 == e.tornFrequency) && (this.tornFrequency = e.tornFrequency);
    ...
    tornpaper() {
        ...
        e.innerHTML = `
            <filter id="${this.filterName}">
                <feTurbulence type="fractalNoise" baseFrequency="${this.grungeFrequency}" result="paper_noise" numOctaves="10" seed="${this.seed}" />
                <feDiffuseLighting in="paper_noise" lighting-color="white" surfaceScale="${this.grungeScale}" result="paper">
                    <feDistantLight azimuth="45" elevation="60" />
                </feDiffuseLighting>
                <feTurbulence baseFrequency="${this.tornFrequency}" type="turbulence" numOctaves="10" seed="${this.seed}" result="edge_noise" />
                <feGaussianBlur stdDeviation="0.5" in="SourceGraphic" />
                <feMorphology operator="erode" radius="5" />
                <feOffset dx="-2" dy="-2" />
                <feDisplacementMap scale="${this.tornScale}" xChannelSelector="B" yChannelSelector="G" in2="edge_noise" result="edge" />
