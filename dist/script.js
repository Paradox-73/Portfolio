...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let activeTabIndex = 3; // Assuming the default active tab is the 4th one (index 3)

  function switchTabs(bgId, fgId, tabElement) {
      // Hide all background and foreground contents
      document.querySelectorAll('.tab_content').forEach(content => content.style.display = 'none');
      // Show the selected background and foreground contents
      document.getElementById(bgId).style.display = 'block';
      document.getElementById(fgId).style.display = 'block';
  ...
  function navigateTabs(direction) {
      ...
      const bgId = activeTab.getAttribute('data-bg-id');
      const fgId = activeTab.getAttribute('data-fg-id');
      switchTabs(bgId, fgId, activeTab);
  }
  ...
  updateTabIndicator();
  switchTabs('bg_home', 'fg_home', tabs[activeTabIndex]);
});
